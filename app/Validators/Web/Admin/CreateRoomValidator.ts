...
export default class CreateRoomValidator {
  ...
  public schema = schema.create({
    name: schema.string({ trim: true }, [rules.maxLength(25), rules.unique({ column: 'name', table: 'rooms' })]),
    block: schema.string({ trim: true }, [rules.maxLength(10)]),
    floor: schema.string({ trim: true }, [rules.maxLength(10)]),
  })
